#include "Atm_button_IO.h"

// Add option for button press callback (for reading i2c buttons etc)

Atm_button_custom & Atm_button_custom::begin( int attached_pin ) {
  // clang-format off
  ...
  Atm_button_custom::pinModeCustom( pin, INPUT_PULLUP );
  return *this;
}

void Atm_button_custom::pinModeCustom(uint8_t pin, PinMode mode){
    pinMode(pin, mode);
}

PinStatus Atm_button_custom::digitalReadCustom(uint8_t pin){
    return digitalRead(pin);
}

int Atm_button_custom::event( int id ) {
  switch ( id ) {
    case EVT_LMODE:
    ...
    case EVT_PRESS:
      return !Atm_button_custom::digitalReadCustom( pin );
    case EVT_RELEASE:
      return Atm_button_custom::digitalReadCustom( pin );
    case EVT_COUNTER:
...
void Atm_button_custom::action( int id ) {
  int press;
  switch ( id ) {
    case ENT_PRESS:
...
Atm_button_custom& Atm_button_custom::onPress( atm_cb_push_t callback, int idx /* = 0 */ ) {
  onpress.set( callback, idx );
  return *this;
}

Atm_button_custom& Atm_button_custom::onPress( Machine& machine, int event /* = 0 */ ) {
  onpress.set( &machine, event );
  return *this;
}

Atm_button_custom& Atm_button_custom::onPress( int id, atm_cb_push_t callback, int idx /* = 0 */ ) {
  if ( id == 1 || id == 2 )
    longpress[id-1].set( callback, idx );
  return *this;
}

Atm_button_custom& Atm_button_custom::onPress( int id, Machine& machine, int event /* = 0 */ ) {
  if ( id == 1 || id == 2 )
    longpress[id-1].set( &machine, event );
  return *this;
}

Atm_button_custom& Atm_button_custom::onRelease( atm_cb_push_t callback, int idx /* = 0 */ ) {
  onrelease.set( callback, idx );
  return *this;
}

Atm_button_custom& Atm_button_custom::onRelease( Machine& machine, int event /* = 0 */ ) {
  onrelease.set( &machine, event );
  return *this;
}

Atm_button_custom& Atm_button_custom::debounce( int delay ) {
  timer_debounce.set( delay );
  return *this;
}

Atm_button_custom& Atm_button_custom::longPress( int max, int delay ) {
  longpress_max = max;
  counter_longpress.set( longpress_max );
...
Atm_button_custom& Atm_button_custom::repeat( int delay /* = 500 */, int speed /* = 50 */ ) {
  timer_delay.set( delay );
  timer_repeat.set( speed );
  return *this;
}

Atm_button_custom& Atm_button_custom::autoPress( int delay, int press /* = 1 */ ) {
  auto_press = press;
  timer_auto.set( delay );
  return *this;
}

Atm_button_custom& Atm_button_custom::trace( Stream& stream ) {
